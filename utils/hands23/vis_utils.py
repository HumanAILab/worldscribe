...
def draw_hand_mask(im, draw, bbox, side, contact, grasp, mask, width, height, font, scale, use_simple):
    ...
    if not use_simple:
        ...
        txt2 = f'{side}'
        # txt1_width, txt1_height = draw.textsize(txt1, font) TODO: uncomment
...
def draw_firstobj_mask(im, draw, bbox, touch, mask, width, height, font, scale, use_simple):
    ...
    if not use_simple:
        txt = touch
        draw = ImageDraw.Draw(im)
        # txt_width, txt_height = draw.textsize(txt, font) TODO: uncomment it
...
def vis_per_image(im, preds, filename, masks_dir, font_path='./utils/hands23/times_b.ttf', use_simple=False):
