...
from utils.gesture.AugmentCE2P import resnet101
from utils.gesture.NewAugmentCE2P import resnet18
import cv2
import numpy as np
...
class ObjectInHand(nn.Module):
    def __init__(self, arch="resnet18", num_classes=20, tcount=False) -> None:
        ...
        if arch == "resnet101":
            self.human = resnet101(num_classes=num_classes, pretrained=None)
        elif arch == "resnet18":
            self.human = resnet18(num_classes=num_classes, pretrained=None)
